...
void render(){
	...
	transformMat = glm::scale(transformMat, glm::vec3(fabs(sin(currentTime)), fabs(sin(currentTime)), 1.0f));
	transformMat = glm::rotate(transformMat, currentTime, glm::vec3(0.0, 1.0, 0.0f));
	// uniform location, how many, transpose?, data
	glUniformMatrix4fv(glGetUniformLocation(shaderProgram->ID, "transformMat"), 1, GL_FALSE, glm::value_ptr(transformMat));
	glBindVertexArray(VAO);
