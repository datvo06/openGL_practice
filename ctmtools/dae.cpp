...
void InsertVertNormalTexcoord(vector<CTM::Vector3>& vertVector,vector<CTM::Vector3>& normalVector,vector<CTM::Vector2>& texcoordVector, bool hasVerts, bool hasNormals, bool hasTexcoords,const string& vertSource ,const string& normalSource ,const string& texcoordSource ,size_t vertIndex , size_t normalIndex , size_t texcoordIndex, map<string, Source >& sources,map<string, vector<Input> >& vertices)
{
  if (hasVerts) {
    ...
    vertVector.push_back(CTM::Vector3(x,y,z));
  }
  ...
  if (hasNormals) {
    ...
    normalVector.push_back(CTM::Vector3(x,y,z) );
  }
  ...
  if (hasTexcoords) {
    ...
    texcoordVector.push_back(CTM::Vector2(s,t));
  }
}

/// Import a DAE file from a file.
void CTM::Import_DAE(const char * aFileName, Mesh * aMesh)
{
...
void CTM::Export_DAE(const char * aFileName, Mesh * aMesh, Options &aOptions)
{
  // Start by ensuring that we use proper locale settings for the file format
  setlocale(LC_NUMERIC, "C");
